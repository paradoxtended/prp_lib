-- This code is basically ox_lib progress script, everything is copied here, so you don't have to edit anything in ox_lib files
...
local function startProgress(data)
    ...
    if data.prop then
        playerState:set('prp:progressProps', data.prop, true)
    end
    ...
    if data.prop then
        playerState:set('prp:progressProps', nil, true)
    end
...
---@param data ProgressProps
---@return boolean?
function prp.progressCircle(data)
    while progress ~= nil do Wait(0) end
...
AddStateBagChangeHandler('prp:progressProps', nil, function(bagName, key, value, reserved, replicated)
